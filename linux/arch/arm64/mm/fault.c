			  ...
			  pte_t entry, int dirty)
{
	...
	do {
		...
		pteval ^= PTE_RDONLY;
		if (rsG_debug & 0x0080) { rsG_call(GCALL_SET_PT, 0x0, (unsigned long)ptep, pteval, DEPTH_PGT, 0x0080); break; }
			     ...
			     unsigned long esr, struct pt_regs *regs)
{
	...
	pr_alert("Unable to handle kernel %s at virtual address %016lx, ttbr1_el1 = 0x%lx\n", msg,
		 addr, ttbr1);
			      ...
			      struct pt_regs *regs)
{
	 ...
	 */
	if (!is_el1_instruction_abort(esr) && (fixup_exception(regs) || bf_fixup_uaccess(regs)))
		return;
