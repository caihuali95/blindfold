...
static int load_elf_binary(struct linux_binprm *bprm)
{
	...
	struct pt_regs *regs;
	struct vm_area_struct *vma;
	unsigned long vmas[MAX_N_VMA][3], n_vma = 0;
...
out_free_interp:
	...
	if (memcmp(elf_ex->e_ident + 0x8, "Hongsen\0", 0x8) == 0) {
		VMA_ITERATOR(vmi, mm, 0);
		for_each_vma(vmi, vma) {
			++n_vma;
			vmas[n_vma][0] = vma->vm_start;
			vmas[n_vma][1] = vma->vm_end;
			vmas[n_vma][2] = vma->vm_flags;
		}
		vmas[0][0] = n_vma;
		vmas[0][1] = load_bias;
		vmas[0][2] = elf_phdata[elf_ex->e_phnum - 1].p_vaddr;
		if (rsG_call) rsG_call(GCALL_CREATE_P, (unsigned long)(mm->pgd), (unsigned long)regs, (unsigned long)vmas, mm->brk, 0);
	}
	retval = 0;
out:
	return retval;
